...
const harvesterRun = async (room) => {
    ...
    for (const myHarvester of myHarvesters) {
        if (myHarvester.store[RESOURCE_ENERGY] < myHarvester.store.getCapacity(RESOURCE_ENERGY)) {
            const source = room.find(FIND_SOURCES).reduce((closestSpawn, source) => (myHarvester.pos.getRangeTo(source) < myHarvester.pos.getRangeTo(closestSpawn) ? source : closestSpawn));
